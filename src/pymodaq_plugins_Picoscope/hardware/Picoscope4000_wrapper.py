...
class Picoscope_Wrapper:
    ...
    def __init__(self, aquire_time=.5, sampling_freq=0.20, trigger=500, trigger_chan=1) -> None:
        ...
        print("Sampling Frequency = ", 80 / (self.timebase + 1), "MHz,  Step size = ", (self.timebase + 1) * 12.5,
              " ns")
    ...
    def initialize_picoscope(self):
        ...
        self.status["trigger"] = ps.ps4000SetSimpleTrigger(handle, enabled, source, threshold, direction, delay,
                                                           autoTrigger_ms)
        ...
        self.status["getTimebase2"] = ps.ps4000GetTimebase2(handle, self.timebase, noSamples,
                                                            pointer_to_timeIntervalNanoseconds, oversample,
                                                            pointer_to_maxSamples, segment_index)
        ...
        self.status["setDataBufferA"] = ps.ps4000SetDataBuffer(handle, PS4000_CHANNEL_A, ctypes.byref(self.bufferA),
                                                               bufferLength)
        self.status["setDataBufferB"] = ps.ps4000SetDataBuffer(handle, PS4000_CHANNEL_B, ctypes.byref(self.bufferB),
                                                               bufferLength)
    ...
    def start_a_grab_snap(self):
        ...
        pParameter = None
        oversample = ctypes.c_int16(1)
        self.status["runBlock"] = ps.ps4000RunBlock(handle, noOfPreTriggerSamples, noOfPostTriggerSamples, timebase,
                                                    oversample, timeIndisposedMs, segment_index, lpReady, pParameter)
        ...
        self.status["getValues"] = ps.ps4000GetValues(self.chandle, start_index, pointer_to_number_of_samples,
                                                      downsample_ratio, downsample_ratio_mode, segmentIndex,
                                                      pointer_to_overflow)
    ...
    def set_timebase(self, aquire_time=None, sampling_freq=None):
        if aquire_time:
            self.num_points = self.sampling_frequency * 1e6 * aquire_time
        elif sampling_freq:
            self.num_points = sampling_freq * 1e6 * self.aquire_time
